(function ($) {
	...
	class Selection{
		...
		selectForPlayback(selectorStr){
			// find motifs and leadins in current section or outside sections
			let selectors = this.stringToArray(selectorStr);
			defaultInstance.motifs.forEach(obj => {
				selectors.forEach(selector => {
					let inThisSection = obj.section == defaultInstance.currentSection;
					let inNoSection = !obj.section;
					if((inThisSection || inNoSection) && inArray(selector, obj.parameters.classList)){
		...
		stringToArray(str){
			...
			switch(typeof str){
				...
				arr = str.split(delimiter).map(item => item.trim());
				break;
        ...
        searchAll(selector, container){
            ...
            allObjects.some(obj => {
                ...
                switch(type){
                    ...
                    case "class":
                    var matchedClass = selector == "*" ? true : inArray(selector, obj.tags);
	...
	class VoiceController {
		...
		addVoiceObject(name, priority, gainObject, fadeTime = this.fadeTime, musicalObject){
			let voiceNames = name.split(" ").map(str => str.trim());
			this.voices.push(new VoiceObject(this.counter, voiceNames, priority, gainObject, fadeTime, musicalObject));
			return this.counter++;
		}
		...
		playVoiceObject(id, startTime, endTime, voiceGroups){
			...
			if(voiceObject){
				...
				this.getVoiceGroup(voiceGroups || voiceObject.groups, id).forEach(obj => {
					...
					try{
						let visualElements = obj.musicalObject.visualElements;
						...
						console.warn("No visual elements for object", obj)
					}
	...
	class VoiceObject {
		constructor(id, groups, priority, gainObject, fadeTime, musicalObject){
			this.id = id;
			this.groups = groups;
	...
	class GUI {
		...
		constructor(target = document.body){
			...
			iMus.instances.forEach(inst => {
				...
				// classify trigger buttons as normal sections or interludes
				this.sectionTriggerButtons.forEach(btn => {
					let selector = btn.getAttribute("selector");
					if(selector.includes("-")){
						let targetSections = selector.split("-");
						if(targetSections.length == 2){
							if(sectionTags.includes(targetSections[0]) && sectionTags.includes(targetSections[1])){
								btn.classList.add("interlude");
							}
				...
				el.addEventListener("change", e => {
					waxml.setVariable("click_volume", e.target.checked ? 1 : 0);
				});
		...
		indicateTriggerButtons(tag, isInterlude){
			...
			if(targetBtn){
				...
				targetBtn.classList.add("active");
				if(isInterlude){
					targetBtn.classList.add("interlude");
				}
			}
		}
	...
	function getTimeSign(ts, defTimeSign){
		...
		if(typeof ts === "string"){
			...
			switch(ts){
				...
				// if only a number of bars are provided
				if(tsArr.length < 2){
					// translate to beats and denominator if timeSign is provided
					if(defTimeSign){
						tsArr[0] *= defTimeSign.nominator;
						tsArr[1] = defTimeSign.denominator;
					} else {
						tsArr[1] = 1;
					}
	...
	function divisionToTime(div, ts, beatDuration){
		...
		if(div == "off"){
			...
			return 60 * 60 * 24 * 365;
		} else if(div.includes("ms")){
			// milliseconds
			return parseFloat(div)*1000;
		} else if(div.includes("s")){
			// seconds
			return parseFloat(div);
		} else {
			// number of bars or division
	...
	function lengthToPos(val, ts){
		ts = ts || this.parameters.timeSign;
		let div = getTimeSign(val, ts);
		let beats = div.nominator / (div.denominator / ts.denominator);
		let bars = Math.floor((beats) / ts.nominator);
		beats %= ts.nominator;
		return `${bars+1}.${beats+1}`;
	}

	function posToLength(){

		let arr = val.split(".");
		let len = {};
		len.bar = parseInt(arr[0] ? arr[0] : 0);
		len.beat = parseInt(arr[1] ? arr[1] : 0);
		len.subBeat = parseInt(arr[2] ? arr[2] : 0);
	...
	function playSound(obj, time, callBackOnStart, callBackOnFinish, track, crop = 0) {
		...
		if(typeof obj.url === "object"){
			...
			if(obj.parameters.activeVariations){
				...
				i = obj.parameters.activeVariations[obj.rndID];
				url = obj.url[i];
				if(!url){
					i = 0;
				...
				if(request.status != 200) { // analyze HTTP status of the response
					...
					audioContext.decodeAudioData(request.response, function(buffer) {
						if (buffer) {
						  ...
						  waxml.getPeaks(obj.url).then(peaks => peaksContainer[obj.url] = peaks);
	...
	*/
	...
	var iMus = function(o, b) {
		...
		this.addSection = function(){
			...
			if(!params.urls){
				if(args.length){
					params.urls = args.filter(el => typeof el != "undefined");
				}
			}
		...
		*/
			...
			if(self.currentSection){
				...
				if(beatLength-timeInBeat <= timeWindow){
					if(self.currentSection.curBeat != curBeat){
						...
						if(nextBarTime == nextClickTime && barTime > currentTime){
							waxml.start("#click_bar", {time:barTime});
							// console.log("click_bar", barTime.toFixed(2), currentTime.toFixed(2));
							waxml.visualize({
								class: "barline",
								pos: barTime
							});
					...
					if((track.active > 0 && track.playing && !track.parameters.fadeTime) || track.parameters.fadeTime){
						...
						for(partID in track.parts){
							...
							if(!(targetPart.playing || targetPart.trigging) || newLoop){
								...
								// I've seen to have abandoned offset in favour of pos. I really need to sort out
								// the use of pos, offset and delay!
								...
								// var partShouldNotPlay = (loopID < 0 && targetPart.pos > 0) || loopID < -1;
								var partShouldNotPlay = loopID < 0 && targetPart.pos > 0;

								var hitInNextLoop = timeInLoop <= posInNextLoop && (timeInLoop + timeWindow) > posInNextLoop;
								// console.log(`hitInNextLoop: ${hitInNextLoop}`);
								// console.log(`hit: ${hit}`);
								// console.log(`partShouldNotPlay: ${partShouldNotPlay}`);
								// console.log(`posInNextLoop: ${posInNextLoop}`);
								...
								if((hit || hitInNextLoop || fadeTrackNeedsTrigging) && loopID >= -1 && !partShouldNotPlay){
		...
		var Section = function(o){
			...
			this.play = function(options = {}, nextTime, selector){
				...
				if(self.currentSection && self.playing) {
					...
					nextTime = nextTime || getNextTime(self.currentSection, this, selector || options.interludeSelector);
				...
				//console.log("play(section " + this.id + ", " + Math.floor(self.sectionStart*100)/100 + ")");
	 			...
	 			if(options.nrOfLoops > 0){

					// does this really work??
					// nrOfLoops used to be the first argument
					// but seams to have been swapped into an object...
					...
					setTimeout(() => {
						let isInterlude = this.tags.includes(options.interludeSelector);
						let tag = isInterlude ? options.interludeSelector : selector ? selector : this.tags[0];
						iMus.GUI.indicateTriggerButtons(tag, isInterlude);
		...
		Section.prototype.lengthToPos = lengthToPos;
		...
		var Track = function(o, section){
			...
			if(this.parameters.voice){
				this.parameters.voiceObjectID = iMus.voiceController.addVoiceObject(this.parameters.voice, 0, this.bus.voiceGain, this.parameters.fadeTime, this);
			}
		...
		Track.prototype.lengthToPos = lengthToPos;
		...
		var Part = function(o, defaultData, bus, curPos){
			...
			*/
			...
			let delay, upbeat;
			if(o.delay){
				delay = divisionToTime(o.delay, defaultData.timeSign, beatDuration);
			}
			if(o.upbeat){
				upbeat = divisionToTime(o.upbeat, defaultData.timeSign, beatDuration);
			}
		...
		var Motif = function(o, section){
			...
			if(this.parameters.voice){
				this.parameters.voiceObjectID = iMus.voiceController.addVoiceObject(this.parameters.voice, 1, this.bus.voiceGain, this.parameters.fadeTime/1000, this);
			}
			...
			for(var urlID in o.urls){
				...
				if(typeof url === "string"){
					...
					obj.offset = this.offset || 0; 
					obj.offset = obj.pos ? this.parentObj.divisionToTime(obj.pos) : obj.offset;
...
*/
	...
	iMus.prototype.lengthToPos = lengthToPos;
			...
			if(selection.objects.length){
				...
				if(selection.sections.length){
					...
					if(interludeSection && iMus.isPlaying()){
						...
						console.log(`interludeLength: ${interludeLength}`);
	...
	iMus.trig = (selectorStr, options = {}, arg2, arg3) => {
		selectorStr.split(",").map(selector => {
			iMus.play(selector.trim(), options, arg2, arg3);
		});
	}
...
*/
	...
	Data.parseMotif = function(motif, section){
		...
		let motifObj;
		let parent = section || self;
		if(motif.nodeName == "motif"){
			motifObj = parent.addMotif(params, urls);
		} else {

			// leadin default values
			if(typeof params.upbeat == "undefined" && typeof params.pos == "undefined" && typeof params.delay == "undefined"){
			...
			motifObj = parent.addLeadIn(params, urls);
		}
	  	...
	  	if(root){
			...
			this.tags = [];
		  	...
		  	arrangements.forEach((arr, _index) => {
				...
				let motifs = arr.querySelectorAll("motif, leadin");
				motifs.forEach(motif => Data.parseMotif(motif, section));
			});

			// add support for non-section motifs
			let motifs = [...root.children].filter(el => el.nodeName == "motif" || el.nodeName == "leadin");
			motifs.forEach(motif => Data.parseMotif(motif));
	...
	console.log("iMusicXML is installed. Version 0.91.15");
	var expectedSchemaFile = "https://momdev.se/lindetorp/imusic/scheme_1.1.28.xsd";
...
2024-09-11
Tillåt ms, s, division, bar och beat för alla längder
