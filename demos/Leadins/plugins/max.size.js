(function () {
	...
	document.querySelectorAll("*[class]").forEach(el => {
		...
		el.classList.forEach(cl => {
			...
			// extend similar to bootstrap translate-middle
			switch(cl){
					 
				case "translate-top-left":
				el.style.transform = "translate(0%,0%)";
				break; 

				case "translate-top-middle":
				el.style.transform = "translate(-50%,0%)";
				break; 

				case "translate-top-right":
				el.style.transform = "translate(-100%,0%)";
				break; 



				case "translate-middle-left":
				el.style.transform = "translate(0%,-50%)";
				break; 
				
				case "translate-middle-right":
				el.style.transform = "translate(-100%,-50%)";
				break;


				
				case "translate-bottom-left":
				el.style.transform = "translate(0%,-100%)";
				break; 
				
				case "translate-bottom-middle":
				el.style.transform = "translate(-50%,-100%)";
				break; 
				
				case "translate-bottom-right":
				el.style.transform = "translate(-100%,-100%)";
				break; 

			}
	
			// add unique max.size classes	
			let arr = cl.split("-");
			let val = arr.pop();
			let key = arr.join("-");
			let prefix = "";
			...
			el.style[key] = `${prefix}${val}${suffix}`;
		});
		if(absPos){
