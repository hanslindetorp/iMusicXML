			...
			Object.keys(selectGroups).forEach(key => {
				...
				switch (range.type) {
					case "number":
						...
						el.setAttribute("min", range.min);
						el.setAttribute("max", range.max);
						el.setAttribute("value", range.min);
						...
						numOutput.innerHTML = range.min;
						iMusic.select(key, range.min);
		...
		Section.prototype.resetFades = function(){
			var targetFilter = defaultInstance.selectFilter.forEach(filter => {
				this.tracks.forEach(track => {
					...
					let state = track.soloGroups.length ? getSoloState(track.soloGroups, filter.name, filter.value) : true;
					if(state){
...
*/
	...
	*/
	...
	iMus.select = function(key, value){
		...
		let numVal, valIsNum, curNumVal, curValIsNum, relativeNum;
		var targetFilter = defaultInstance.selectFilter.find(curFilter => curFilter.name == key);
		if(!targetFilter){
			targetFilter = {name: key};
			defaultInstance.selectFilter.push(targetFilter);
		}
		if(typeof value == "string"){
			numVal = parseFloat(value);
			valIsNum = !isNaN(numVal);
			curNumVal = parseFloat(targetFilter.value);
			curValIsNum = !isNaN(curNumVal);
			relativeNum = value.includes("+") || value.includes("-");
		}

		if(relativeNum && valIsNum && curValIsNum){
			targetFilter.value = curNumVal + numVal;
		} else {
	...
	if(window.webAudioXML){
  	...
  	class Range {
		...
		get min() {
			...
			if(val instanceof MinMax){val = val.min}
			return val;
		}

		get max(){
			let val = this.values[this.values.length-1];
			if(val instanceof MinMax){val = val.max}
			return val;
		}
